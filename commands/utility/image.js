...
module.exports = class ImageCommand extends Command{
    ...
    run(message, {image}){
        let banner = this.client.provider.get(message.guild.id, "welcome_image", null);
        if (banner == null){
    ...
    return new Promise((resolve, reject) => {
        ...
        Jimp.read(mask).then((mask) => {
            ...
            if (image == 'profile'){
                ...
                if(image == null){
                    image = member.user.defaultAvatarURL;
                }
            }
            Jimp.read(image).then((profile) => {
                ...
                Jimp.read(background).then((banner) => {
                    ...
                    addfontselamatdatang(banner, member).then(banner => {
                        resolve()
                    })
                }).catch(function (err) {
                    console.error(err);
